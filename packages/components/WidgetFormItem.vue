...
export default {
  ...
  methods: {
    getComponent (type, component) {
      ...
      else if (['dates', 'date', 'datetime', 'datetimerange', 'daterange', 'week', 'month', 'year'].includes(type)) { result = 'date' } else if (type === 'cascader') result = 'cascader'
    ...
    getPlaceholder (item) {
      ...
      if (['select', 'checkbox', 'radio', 'tree', 'color', 'dates', 'date', 'datetime', 'datetimerange', 'daterange', 'week', 'month', 'year', 'map', 'icon'].includes(item.type)) { return `请选择 ${label}` } else return `请输入 ${label}`
    }
  }
}
</script>
