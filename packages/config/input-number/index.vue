<template>
  <div>
    ...
    <el-form-item v-loquat-has-perm="[plugin, 'minRows']" label="最小值">
      <el-input-number v-model.number="plugin.minRows"/>
    </el-form-item>
    <el-form-item v-loquat-has-perm="[plugin, 'maxRows']" label="最大值">
      <el-input-number v-model.number="plugin.maxRows"/>
    </el-form-item>
    <el-form-item v-loquat-has-perm="[plugin, 'step']" label="步长">
      <el-input-number v-model.number="plugin.step"/>
    </el-form-item>
    <el-form-item v-loquat-has-perm="[plugin, 'precision']" label="精度">
      <el-input-number v-model.number="plugin.precision"/>
    </el-form-item>
    <el-form-item v-loquat-has-perm="[plugin, 'controlsPosition']" label="控制按钮位置">
      <el-radio-group v-model="plugin.controlsPosition">
        <el-radio-button label="">默认</el-radio-button>
        <el-radio-button label="right">右侧</el-radio-button>
      </el-radio-group>
    </el-form-item>
    <el-form-item v-loquat-has-perm="[plugin, everyPermission.defaultValue, 2]" label="默认值">
      <el-input-number v-model.number="plugin.value"
                       :step="plugin.step"
                       :min="plugin.minRows"
                       :max="plugin.maxRows"
      />
